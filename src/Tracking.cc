...
{
    mImGray = im;                                       // 输入左目RGB或RGBA图像
    // 步骤1：将RGB或RGBA图像转为灰度图像                    // 1、将图像转为mImGray并初始化mCurrentFrame
    if(mImGray.channels()==3)                           // 2、进行tracking过程
...
{
    if(mState==NO_IMAGES_YET)                               // mState为tracking的状态机
    {                                                       // SYSTME_NOT_READY, NO_IMAGE_YET, NOT_INITIALIZED, OK, LOST
        mState = NOT_INITIALIZED;                           // 如果图像复位过、或者第一次运行，则为NO_IMAGE_YET状态
    }
    ...
    if(mState==NOT_INITIALIZED)  // 步骤1：初始化
    {
    ...
    else  // 步骤2：跟踪 //初始化之后一直执行这段代码,不出问题的情况下，第三帧及后续帧都进入此分支
    {
        ...
        if(!mbOnlyTracking)  //表示正常VO模式（有地图更新）
        ...
        if(bOK)
        {
            ...
            // Clean VO matches：清除UpdateLastFrame中为当前帧临时添加的MapPoints
            for(int i=0; i<mCurrentFrame.N; i++)
            {
            ...
            //在当前帧中将这些MapPoints剔除，这里从MapPoints数据库中删除.提高双目或rgbd摄像头的帧间跟踪效果，用完以后就扔了，没有添加到地图中
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        {   //剔除三角测量失败的匹配点
            ...
            // Set Frame Poses //初始帧pose设置为单位矩阵  //更新当前帧pose
